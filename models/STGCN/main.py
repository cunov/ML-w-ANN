...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if validation_losses[-1] < best_val_score:
            best_val_score = validation_losses[-1]
            print('Saving model with loss: {}'.format(best_val_score))
            torch.save(net.state_dict(), 'models/STGCN/state_dict')


        checkpoint_path = "models/STGCN/checkpoints/"
        if not os.path.exists(checkpoint_path):
            os.makedirs(checkpoint_path)
        with open("models/STGCN/checkpoints/losses.pk", "wb") as fd:
